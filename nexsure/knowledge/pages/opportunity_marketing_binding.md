...
6.7 The terminal action: "In Force" confirmation modal (documented, NOT submitted)
...
Buttons: "In Force" (blue, the actual irreversible confirm) and "Cancel" (red outline, used to close the modal safely in this test).

This was intentionally NOT confirmed. Clicking "In Force" is the one action in this entire Marketing → Binding flow that converts a quote into a real policy record — treat it exactly like Send Application / Lost Opportunity: automate only after explicit business confirmation, and expect it to be effectively irreversible in a production-like environment.
