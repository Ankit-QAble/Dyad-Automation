...
- What clicking Export produces (file format, contents) — not triggered (would download a file). The "+ New" button's flow **is now documented** — see "Page: New Opportunity — Select Client" below.
...
## Page: New Opportunity — Select Client (`Find a Client` / `New Client`)

| Fact | Value |
|---|---|
| Reached via | Opportunities list → `+ New` button |
| Screen | "Opportunities: New" wizard, step **1. Select Client** (of 3: Select Client, Select Assignment, Select Product) |
| `<title>` | Not re-checked here — see Open items |

### Overview

Clicking `+ New` on the Opportunities list opens a 3-step "Opportunities: New" wizard. Step 1, "Select Client," starts empty (placeholder skeleton fields) with three buttons: `Find Client`, `New Client`, `Next` (`Next` disabled until a client is chosen). `Find Client` opens a search modal; if the search comes up empty, that same modal offers a `New Client` button, which exits the modal and opens a separate full-page "Clients: New" wizard.

### Test case: searching for a client that doesn't exist

| Step | Input | Expected / observed result |
|---|---|---|
| Search for a client not in the system | Client Name (new, not existing), e.g. `QAble_123` | System shows **"No Results Found"** — no matching client displayed |
| Click **New Client** | — | The **"Clients: New"** wizard opens (full page, not a modal), landing on step 1, **Client Info** |

Note: the actual button is labeled **`New Client`**, not "Create Client" as in the original test note — same action, different label. Worth using the exact label in automation locators.

### `Find a Client` modal — element inventory

| Element | Type | Notes |
|---|---|---|
| Keywords | text input | Free-text client-name search; results appear to update live/near-live as you type. |
| Branch | combobox, value `Any` | Reuses the **same 85-entry tenant branch list** seen in the Opportunities filter panel — confirms this is a shared component, not page-specific. |
| Retail Agent | combobox, value `Any` | **Not** a static enumerable list — opens showing only `Type to search` (placeholder) and `Any`; it's a type-ahead search, not a fixed dropdown. |
| No-match state | text + button | Heading `No Results Found`, then a `New Client` button. |
| Match state | list | Heading `Search Results`, then one row per match: entity icon + name + `(C)`/`(P)`-style type marker + city, state (e.g. "aloknov (C) San Jose, CA"). Rows are clickable. |

Selecting a matching result closes the search and populates Step 1 with a client summary card:

| Element | Type | Notes |
|---|---|---|
| Client: {name} ({type}) | static text | e.g. `Client: aloknov (C)`. |
| OFAC Check | link (opens elsewhere) | Sanctions/compliance check — not followed in this pass. |
| OFAC status | combobox | Full option list: **Pending, Completed, Blocked, Review Required**. ⚠️ This reflects the client's real compliance-review status — do not change it while just exploring; it wasn't changed in this pass. |
| Address | static text | Client's primary address. |
| Contact | link | Associated contact name. |
| NAICS | static text / link | If missing, shows a prompt ("please add a NAICS classification") plus an `Update Client` link. |
| Change Client / New Client / Next | buttons | `Next` becomes enabled once a client is selected. |

### `New Client` wizard — "Clients: New"

A separate full-page 3-step wizard (**1. Client Info, 2. Client Contacts, 3. Assignment**), distinct from the Opportunity wizard that launched it.

**Step 1 — Client Info:**

| Element | Type | Notes |
|---|---|---|
| Client Type* | two-option toggle | **Commercial** / **Personal**. Changes the form: Commercial adds a `Website` field and a `Business Classification` (NAICS code table: Primary / NAICS Code / Description) panel; Personal has neither. |
| Client Name* | text input | **Pre-fills from the keyword typed in the "Find a Client" search** (observed: searching "aloknov" and hitting no results, then New Client, pre-filled Client Name with "aloknov"). |
| Website | text input | Commercial only. |
| Location Type* | combobox, dependent on Client Type | **Commercial** list: Corporate Headquarters, General Office, Manufacturing/Plant, Warehouse, Distribution Center, Retail Location, Accounting Office, Mailing Address, DELETED/UNUSED/INACTIVE/PRIME Commercial Location, Risk Location. **Personal** list: Home Office, Rental Property, Primary Residence, Vacation Residence, CRM QA Office, Mailing Address, Travel Station, INACTIVE/UNUSED/DELETED/PRIME Personal Location, Risk Location. |
| Location Name | text input, info tooltip | Optional. |
| Street Address* | text input | |
| Unit | text input | Optional. |
| City* | text input | |
| State/Province* | combobox | Standard USPS-style state/territory list (~108 entries incl. a "[USA]" group header), e.g. Alabama, Alaska, American Samoa, Arizona, … — not tenant-specific, not reproduced in full here. |
| Zip/Postal* | text input | |
| Country | combobox, default `United States` | |
| International Address Info | text input | |
| Phone / Fax | text inputs | |
| Same as Mailing Address | checkbox | Checked by default. |
| Next | button | Runs client-side validation (see Behavior). |

### Behavior & persistence

- **Client Type changes the form**, not just a label: Commercial reveals Website + Business Classification (NAICS); Personal does not. Both share the same address block, but **Location Type's option list depends on which Client Type is selected** — re-query it after switching rather than caching one list for both.
- **The Find-a-Client search keyword carries over** into the New Client form's Client Name field — useful for automation (typing the intended name once may be enough), but also means the field isn't blank by default when reached via a failed search.
- **Client-side validation on Next:** empty required fields get a red outline and an inline `Please provide` message under each one; the wizard does not advance and nothing is saved. Only genuinely required fields were flagged (Client Name was not, since it was pre-filled).
- Nothing was saved/submitted in this pass — Client Contacts (step 2) and Assignment (step 3) were not reached, and no client or opportunity record was created.

### Selectors

| Element | Accessible role / label | Fallback CSS |
|---|---|---|
| Find Client button (Step 1, empty state) | button, name `Find Client` | — |
| New Client button | button, name `New Client` | appears in both the empty Step 1 and inside the Find-a-Client modal's no-results state |
| Find a Client — Keywords | text input | inside the `Find a Client` modal |
| Find a Client — Branch | combobox, value `Any` | shares the branch component used on the Opportunities filter panel |
| Find a Client — Retail Agent | combobox, type-ahead | placeholder text `Type to search` |
| Search result row | clickable row | `.searchResults .nex_flexible_grid.clickableRow .grid_row` |
| OFAC status | combobox | next to the `OFAC Check` link on the populated Select Client card |
| Client Type toggle | two buttons, text `Commercial`/`Personal` | |
| Client Name | text input, label `Client Name` | |
| Location Type | combobox, label `Location Type` | options depend on Client Type — see table above |
| State/Province | combobox, label `State/Province` | |

### Open items

- Steps 2 (Client Contacts) and 3 (Assignment) of the New Client wizard.
- Steps 2 (Select Assignment) and 3 (Select Product) of the New Opportunity wizard, and what completing it actually creates.
- What the `OFAC Check` link opens.
- What clicking `Update Client` (from the NAICS prompt) does.
- Whether the Retail Agent type-ahead in "Find a Client" returns tenant-specific agent names (not queried with real text in this pass).
- Whether `<title>` changes on these screens (not checked here — see the running note on page-dependent titles at the top of this document).
